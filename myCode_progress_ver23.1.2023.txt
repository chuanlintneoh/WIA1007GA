...
data <- read_excel("data_obesity_r.xlsx")
mydata <- read_excel("data_obesity_r.xlsx")

mydata$Gender <- as.factor(mydata$Gender)
mydata$Family.history.with.overweight <- as.factor(mydata$Family.history.with.overweight)
mydata$Frequent.consumption.of.high.caloric.food <- as.factor(mydata$Frequent.consumption.of.high.caloric.food)
mydata$Frequency.of.consumption.of.vegetables <- as.factor(mydata$Frequency.of.consumption.of.vegetables)
mydata$Consumption.of.food.between.meals <- as.factor(mydata$Consumption.of.food.between.meals)
mydata$Smoking <- as.factor(mydata$Smoking)
mydata$Consumption.of.water.daily <- as.factor(mydata$Consumption.of.water.daily)
mydata$Calories.consumption.monitoring <- as.factor(mydata$Calories.consumption.monitoring)
mydata$Physical.activity.frequency <- as.factor(mydata$Physical.activity.frequency)
mydata$Time.using.technology.devices <- as.factor(mydata$Time.using.technology.devices)
mydata$Consumption.of.alcohol <- as.factor(mydata$Consumption.of.alcohol)
mydata$Transportation.used <- as.factor(mydata$Transportation.used)
mydata$Type.of.obesity <- as.factor(mydata$Type.of.obesity)
...
Testing = select(Testing, -Height.m., -Weight.kg., -Type.of.obesity, -BMI)

RFM = randomForest(Type.of.obesity ~ Gender + Age + Family.history.with.overweight + 
                     Frequent.consumption.of.high.caloric.food + 
                     Frequency.of.consumption.of.vegetables + 
                     Consumption.of.food.between.meals + Smoking + 
                     Consumption.of.water.daily + Calories.consumption.monitoring 
                   + Physical.activity.frequency + Time.using.technology.devices + 
                     Consumption.of.alcohol + Transportation.used, data = Training)
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "faq",
              ...
              fluidRow(
                box(
                  ...
                  valueBoxOutput("obesityBox",width = 6),
                  valueBoxOutput("categoryBox",width = 6),
              ...
              fluidRow(
                ...
                column(
                  ...
                  box(title = strong("Frequently Asked Questions"),width = NULL,
                      h4("1."),
                      h5("Q: What is obesity?"),
                      h5("A: Overweight and obesity are defined as abnormal or excessive fat accumulation that presents a risk to health. A body mass index (BMI) over 25 is considered overweight, and over 30 is obese."),
                      h4("2."),
                      h5("Q: How is obesity measured?"),
                      h5("A: Obesity is typically measured using body mass index (BMI), which is calculated by dividing a person's weight in kilograms by their height in meters squared."),
                      h4("3."),
                      h5("Q: What are the causes of obesity?"),
                      h5("A: The main causes of obesity are a combination of genetic, environmental, and lifestyle factors. These can include a lack of physical activity, unhealthy diet, and certain medical conditions."),
                      h4("4."),
                      h5("Q: What are the health risks associated with obesity?"),
                      h5("A: Obesity increases the risk of several serious health conditions such as diabetes, heart disease, stroke, and certain types of cancer."),
                      h4("5."),
                      h5("Q: How can obesity be treated?"),
                      h5("A: Obesity can be treated through a combination of diet, exercise, and lifestyle changes, as well as medications and/or weight loss surgery."),
                      h4("6."),
                      h5("Q: How can our obesity prediction model help?"),
                      h5("A: Our obesity prediction model uses machine learning algorithms to analyze data on a person's age, sex, weight, height, and other factors to predict their likelihood of developing obesity. This information can help users make lifestyle changes to reduce their risk of obesity."),
                      h4("7."),
                      h5("Q: Is our obesity prediction model accurate?"),
                      h5("A: Our obesity prediction model implemented with Random Forest algorithm has been trained on large datasets and has been tested for accuracy. However, it is not a diagnostic tool and should not be used to make medical decisions. It is always best to consult with a healthcare professional for a personalized evaluation."),
                      h4("8."),
                      h5("Q: How can I use the prediction model on your website?"),
                      h5("A: You can use our prediction model by providing your personal information such as age, sex, weight, height, and other factors on our website. The prediction model will use this information to give you an estimate of your risk of developing obesity."),
                      h4("9."),
                      h5("Q: Is this website affiliated with any medical institution or organization?"),
                      h5("A: This website is not affiliated with any medical institution or organization. It is intended for informational and educational purposes only.")
                  )
                )
              )
      ),
        ...
        fluidPage(
          ...
          column(
            ...
            box(
              ...
              actionButton("dopredict", "Predict Now!")
            )
          ),
          column(
            width = 4,
            conditionalPanel(condition = "input.dopredict > 0", 
                             box(
                               width = "100%",
                               title = strong("Risk of Obesity"),
      ...
      tabItem(tabName = "group_details",
              fluidRow(
                plotOutput("groupdetailsHeader", height = "50%"),
                box(
                  width = 9,
              ...
              fluidRow(
                box(
                  ...
                  h3("22004874"),
                  h3("\"As the group leader, I am committed to ensuring that our obesity risk assessment product is a comprehensive, accurate, and user-friendly tool that empowers individuals to take control of their health.\"")
                ),
                box(
                ...
                box(
                  ...
                  h3("TNEOH CHUAN LIN"),
                  h3("22004808"),
                  h4("\"A user-friendly, intuitive, and visually appealing interface is crucial in engaging users and encouraging them to utilize our data product effectively.\"")
                ),
                box(
                  width = 4,
                  height = "250px",
                  h3("PANG SZE WEN"),
                  h3("22004873"),
                  h4("\"My passion is to simplify complex data by creating clear and visually compelling representations through the use of engaging graphs, bar charts, and tables.\"")
                ),
                box(
                  width = 4,
                  height = "250px",
                  ...
                  h3("22004876"),
                  h4("\"Users can confidently make well-informed decisions about their health by utilizing our data product, which is endowed with personalized, actionable insights and guidance.\"")
                ),
                box(
                  width = 4,
                  height = "250px",
                  ...
                  h3("22057027"),
                  h4("\"With the use of cutting-edge technology that is publicly available, explorable and reliable, it enables us to provide our users with the most accurate, transparent and personalized assessments.\"")
                )
              ),
              fluidRow(
                width = "100%",
                column(
                  width = 12,
                  HTML("<p style='text-align: center;'>R Shiny @ Github | <a href='https://github.com/chuanlintneoh/WIA1007GA' target='_blank'>github.com</a></p>")
                )
              )
      )
    )#tabitems
...
server <- function(input, output){
  ...
  observeEvent(input$dopredict, {
    ...
    input_data <- data.frame(
      Gender = input$gender,
      Age = input$age,
      Family.history.with.overweight = input$family_history,
      Frequent.consumption.of.high.caloric.food = input$high_caloric_food,
      Frequency.of.consumption.of.vegetables = input$frequency_of_consumption_of_vegetables,
      Consumption.of.food.between.meals = input$consumption_of_food_between_meals,
      Smoking = input$smoking,
      Consumption.of.water.daily = input$consumption_of_water_daily,
      Calories.consumption.monitoring = input$calories_consumption_monitoring,
      Physical.activity.frequency = input$physical_activity_frequency,
      Time.using.technology.devices = input$time_using_technology_devices,
      Consumption.of.alcohol = input$consumption_of_alcohol,
      Transportation.used = input$transportation_used
    )
    
    Testing <- rbind(Testing, input_data)
    testrow <- Testing[nrow(Testing), ]
    
    for (col in colnames(testrow)) {
      if (is.factor(testrow[,col])) {
        testrow[,col] <- factor(testrow[,col], levels = levels(Training[,col]))
      }
    }
    predictions <- predict(RFM, testrow, type="prob")
    
    output$predict <- renderText({
      paste0(predictions[2]*100, "%")
    })
    advice <- reactive({
      if (is.na(predictions[2]) || predictions[2] <= 0){
        return("Something went wrong, please try again.")
      }else if (predictions[2] <= 0.2){
        return("Good job! You are considered to have a low risk for obesity. You have a healthy body mass index (BMI) and may not have any significant risk factors for developing obesity, such as a family history or unhealthy lifestyle habits.")
      }else if (predictions[2] <= 0.4){
        return("It's not good but it's not bad neither. You are considered to have a moderate risk for obesity. You may have some risk factors for developing obesity, such as a family history, sedentary lifestyle, or unhealthy eating habits. You may also have a slightly elevated BMI, but still fall within the normal range. Please take care of yourself!")
      }else {
        return("Uh oh! You are considered to have a high risk for obesity. You would likely have several risk factors for developing obesity, such as a family history, sedentary lifestyle, and unhealthy eating habits. You may also have a BMI that falls into the overweight or obese range. It is important for these individuals to take proactive steps to address their risk factors and reduce their risk for obesity.")
      }
    })
    output$predictadvice <- renderText({
      paste0(advice())
    })
  })
}
